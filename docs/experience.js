...
renderToday=function(){
 ...
 const strokeOffset=213.6*(1-percent/100);
 $('#content').innerHTML=`
 ...
 <div class="momentum-card minimal-progress-bar" role="progressbar" aria-valuenow="${percent}" aria-valuemin="0" aria-valuemax="100">
   <div class="momentum-ring-wrap">
     <svg class="momentum-ring" viewBox="0 0 80 80" aria-hidden="true">
       <circle class="ring-bg" cx="40" cy="40" r="34"/>
       <circle class="ring-fill" cx="40" cy="40" r="34" style="stroke-dasharray:213.6;stroke-dashoffset:${strokeOffset};"/>
     </svg>
   </div>
 ...
 <details class="identity-drawer">
   <summary class="identity-drawer-header">
     <span>${ico('target')} <b>Identity & Mindset</b> · “I’m ${esc(state.identity)}”</span>
   ...
   <div class="tip">${ico('spark')}<div><strong>Make showing up the goal.</strong>Your two-minute version counts. Build the habit, then build on it.</div><button class="icon-button" id="tip-method" aria-label="Explore the method">${ico('arrow')}</button></div>
 </details>`;
 ...
 return `<div class="habit ${done?'done':''}" data-habit="${esc(h.id)}">
  <div class="habit-primary-row">
    ...
    <div class="habit-actions">
      ...
      <button class="icon-button edit" data-edit="${esc(h.id)}" aria-label="Edit ${esc(h.name)}" title="Edit habit">${ico('more')}</button>
    </div>
  </div>
 </div>`;
}
...
renderChat=function(){
 const welcome=`<div class="chat-welcome"><div class="coach-orbit"><span>✦</span></div><h2>A small nudge.<br>A fresh perspective.</h2><p>A thoughtful space to untangle a habit, find your next step, or simply begin again.</p><div class="welcome-prompts">${[['leaf','Find my first small step','Help me choose my first two-minute habit.'],['layers','Build a better routine','Help me connect a new habit to my daily routine.'],['spark','Start again, gently','I missed a day. Help me get back on track.']].map(([i,t,p])=>`<button data-welcome="${esc(p)}">${ico(i)}<span>${t}</span><span>↗</span></button>`).join('')}</div></div>`;
 const chat=$('#chat');chat.innerHTML=`<div class="chat-date">DAILY HABIT COACH</div>${state.messages.length?'':welcome}`+state.messages.map((m,i)=>`<div class="message-label">${m.role==='user'?'You':'✦ Habit Coach'}</div><div class="message ${m.role==='user'?'user':''}">${esc(m.text).replace(/\*\*([^*\n]+)\*\*/g,'<strong>$1</strong>')}</div>${m.role==='model'?`<div class="message-tools"><button data-copy="${i}" aria-label="Copy coach response">${ico('copy')} Copy</button></div>`:''}`).join('');
 document.querySelectorAll('[data-welcome]').forEach(b=>b.onclick=()=>preparePrompt(b.dataset.welcome));
 document.querySelectorAll('[data-copy]').forEach(b=>b.onclick=async()=>{try{await navigator.clipboard.writeText(state.messages[Number(b.dataset.copy)].text);toast('Response copied.');}catch{toast('Copy is unavailable. Select the response text to copy it.');}});chat.scrollTop=chat.scrollHeight;
};
